...
var FilesAPI = class {
  ...
  async upload(input, options) {
    ...
    let fileSize;
    if (typeof input === "string") {
      if (!fs__namespace.existsSync(input)) {
        throw new ValidationError(`File not found: ${input}`);
      }
      const stats = fs__namespace.statSync(input);
      if (!stats.isFile()) {
        throw new ValidationError(`Not a file: ${input}`);
      }
      stream$1 = fs__namespace.createReadStream(input);
      filename = path__namespace.basename(input);
      fileSize = stats.size;
    } else if (Buffer.isBuffer(input)) {
      stream$1 = stream.Readable.from(input);
...
exports.ConversionToolsClient = ConversionToolsClient;
exports.ConversionToolsError = ConversionToolsError;
